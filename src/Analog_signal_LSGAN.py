...
'''
作了如下修改
  1，判别器和生成器是否采用batch_nomor
  2, 判别器采用lrelu激活函数
  3，损失函数采用LSGAN 和WGAN
  4，优化器采用adam或者RMSPdrop
'''
...
class model :
    def __init__(self, params):
        ...
        self.is_adam=params.is_adam
        self.BN_g=params.BN_g
        self.BN_c=params.BN_c
...
# read real data
    ...
    def generator_mlp(self,train,base=8,recover=False):
        ...
        for i in range(len) :
            train= ly.fully_connected(
               train, size[i], activation_fn=None, normalizer_fn=ly.batch_norm if self.BN_g else None,scope='g{}'.format(i+1))
        ...
        train1 = ly.fully_connected(
            train, self.number, activation_fn=None, normalizer_fn=ly.batch_norm if self.BN_g else None,scope='g')
    ...
    def critic_mlp(self,img,base=16,reuse=False):
        ...
        with tf.variable_scope('critic') as scope:
            if reuse:
                scope.reuse_variables()
            for i in range(len):
                img = ly.fully_connected(img, size[i], activation_fn=lrelu, normalizer_fn=ly.batch_norm if self.BN_c else None)
            logit = ly.fully_connected(img, 1, activation_fn=None,normalizer_fn=ly.batch_norm if self.BN_c else None)
    ...
    def build_graph(self):
        ...
        opt_g = ly.optimize_loss(loss=g_loss, learning_rate=learning_rate_ger,
                                 optimizer=partial(tf.train.AdamOptimizer, beta1=0.5,
                                                   beta2=0.9) if self.is_adam is True else tf.train.RMSPropOptimizer,
    ...
    def train(self):
        ...
        path = os.path.join('..','signal_data',self.type)

        label="{}_G_{}_D_{}_{}_{}_{}".format(self.type,"M" if self.is_mlp_g else "C","M" if self.is_mlp_c else "C" ,self.loss,self.number,"Noise" if self.Noise else "")
        label2="_OP_{}_{}{}".format('A' if self.is_adam else "R","G" if self.BN_g else "","D" if self.BN_c else "")
        label=label+label2
        print(label+"  " + str(train_names))
       #模型保存路径和结果保存路径
        model_save=os.path.join('..','save',self.type,label)
        ...
        data_real = np.ones((1, self.number))
        for file in os.listdir(path):
            if os.path.splitext(file)[1] == '.mat':
                ...
                data_real=np.vstack((data_real,data))
                draw_Series_Grid(data,"real_data",result_save,"real_data_{}".format(os.path.splitext(file)[0]),row=3,col=3)
...
def oneSolution(batch_size,is_mlp_g,is_mlp_c,max_iter_step,Noise,loss,type,number,is_adam,BN_g,BN_c):
    ...
    parser.add_argument('--is_adam', type=bool, default=is_adam)
    parser.add_argument('--BN_g', type=bool, default=BN_g)
    parser.add_argument('--BN_c', type=bool, default=BN_c)
                ...
                for is_g in [False]:
                    for is_c in [False]:
                        for loss in ['LSGAN','WGAN']:
                            ...
                            max_iter_step=5001
                            Noise=True
                            train_names,type, label,number, model_save, result_save=oneSolution(batch_size,is_mlp_g,is_mlp_c,max_iter_step,Noise,loss,real_data,number,is_a,is_c,is_g)
                            #恢复数据`
